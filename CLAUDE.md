...
2. **Concepto** — SESIÓN DE LECCIÓN (~20 min): máximo 1 tópico nuevo por sesión, siguiendo el orden del roadmap desde `posicion_actual`. Cada tópico nuevo ocupa DOS sesiones: la de lección (este paso) y la de ejercicio (paso 3), nunca las dos el mismo día. La teoría NO se dicta en el chat: escribe la lección en `ejercicios/fase-N/<topic_id>/leccion.md` (formato en `ejercicios/_plantilla/leccion.md`) con la secuencia herramienta → porqué → demo + 2-4 preguntas de comprensión o predicción, y en el chat di solo: "Lee `ejercicios/fase-N/<topic_id>/leccion.md` y responde las preguntas aquí en el chat para irlas desarrollando." Discute cada respuesta en el chat. Cuando las preguntas estén discutidas, la sesión de lección termina: pide `/end-sesion` sin montar el ejercicio; el cierre deja el tópico en `visto` y `pendiente` = "lección leída, ejercicio no empezado".
   - Excepción única: si el estudiante ya leyó la lección por su cuenta antes de la sesión y lo demuestra respondiendo las preguntas en los primeros minutos, el ejercicio puede ir en la misma sesión.
3. **Ejercicio** — SESIÓN DE EJERCICIO (~20 min): se abre desde el `pendiente` "lección leída, ejercicio no empezado" (recap de ≤8 líneas). Tras los repasos vencidos, crea los archivos del ejercicio y deja trabajar al estudiante el resto de la sesión; revisa cuando te avise. Los ejercicios de repaso, las variantes y el trabajo a medias de una sesión anterior siguen este mismo paso: no hay lección nueva ese día.
...
2. Actualiza `state/progress.json`:
   ...
   - `pendiente`: si el ejercicio quedó a medias, escribe `{topic_id, ejercicio (ruta), paso (dónde quedó: qué pasa y qué no), siguiente (una línea accionable)}`; si la sesión fue de lección (preguntas discutidas, ejercicio no montado), escribe `{topic_id, ejercicio: null, paso: "lección leída y preguntas discutidas; ejercicio no empezado", siguiente: "montar y resolver el ejercicio de <topic_id>"}`; si todo cerró, `null`.
